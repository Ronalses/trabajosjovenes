...
<div id="job-manager-resume-alerts">
    ...
    <div class="table-responsive mt30">
        <table class="job-manager-resume-alerts responsive-table">
            <thead>
                <tr>
            ...
            <tbody>
                <?php foreach ( $alerts as $alert ) : ?>
                    ...
                    <tr class="alert-<?php echo $alert->post_status === 'draft' ? 'disabled' : 'enabled'; ?>">
                        ...
                        <?php endif; ?>
                        <?php if ( get_option( 'resume_manager_enable_skills' ) && wp_count_terms( 'resume_skill' ) > 0 ) : ?>
                            <td class="alert_resume_skill"><?php
                                $term_ids = ! empty( $search_terms['skills'] ) ? $search_terms['skills'] : array();
                                $terms    = array();
                                if ( ! empty( $term_ids ) ) {
                                    $terms = get_terms( array(
                        ...
                        <td class="alert_frequency"><?php
                            ...
                            echo '<small>' . sprintf( esc_html__( 'Next: %s at %s', 'cariera' ), date_i18n( get_option( 'date_format' ), wp_next_scheduled( 'job-manager-resume-alert', array( $alert->ID ) ) ),  date_i18n( get_option( 'time_format' ), wp_next_scheduled( 'job-manager-resume-alert', array( $alert->ID ) ) ) ) . '</small>';
                        ?></td>
    ...
    <a href="<?php echo remove_query_arg( 'updated', add_query_arg( 'action', 'add_alert' ) ); ?>" class="btn btn-main btn-effect mt30"><?php _e( 'Add alert', 'cariera' ); ?></a>
