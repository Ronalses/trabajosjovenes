		...
		<div class="field account-sign-in">
            <p>
                <?php
                    ...
                    printf( esc_html__( 'You are currently signed in as %s.', 'cariera' ), $user->user_login );
                ?>
            </p>
	...
	if ( ! empty( $registration_fields ) ) {
		foreach ( $registration_fields as $key => $field ) {
			...
			<fieldset class="fieldset-<?php echo esc_attr( $key ); ?>">
				<label
					for="<?php echo esc_attr( $key ); ?>"><?php echo esc_html( $field[ 'label' ] ) . wp_kses_post( apply_filters( 'submit_company_form_required_label', $field[ 'required' ] ? '' : ' <small>' . esc_html__( '(optional)', 'cariera' ) . '</small>', $field ) ); ?></label>
				<div class="field <?php if( $field[ 'required' ] ) { echo esc_attr( 'required-field' ); } else { echo esc_attr(''); } ?>">
