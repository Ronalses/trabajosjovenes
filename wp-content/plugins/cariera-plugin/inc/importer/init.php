...
* 
* ========================
* CARIERA ONE CLICK DEMO IMPORTER
* ========================
*     
**/
...
class CarieraDemoImporter extends CarieraThemeImporter {
    ...
    public function __construct() {
		$this->demo_files_path = CARIERA_PATH . '/inc/importer/demo-files/';
        self::$instance = $this;
    ...
    public function run() {
		add_action( 'wp_ajax_cariera_import_form', array( $this, 'ajax_import'), 10, 1 );
		add_action( 'wp_ajax_cariera_set_demo_content', array( $this, 'ajax_set_demo_content') );
    	add_action( 'wp_ajax_cariera_import_theme_options', array( $this, 'ajax_import_theme_options'), 10, 1 );
    	add_action( 'wp_ajax_cariera_import_theme_widgets', array( $this, 'ajax_import_theme_widgets'), 10, 1 );
		add_action( 'wp_ajax_cariera_import_slider', array( $this, 'import_slider') );
    	add_action( 'wp_ajax_cariera_after_import', array( $this, 'ajax_after_import'), 10, 1 );
    	add_action( 'wp_ajax_cariera_require_plugins', array($this, 'ajax_get_require_plugins'));
    }
	...
	public function ajax_after_import() {
		...
		$bookmarks 			= get_page_by_title( 'My Bookmarks' );
		$applied_jobs 		= get_page_by_title( 'Past Applications' );
		$listing_reports 	= get_page_by_title( 'Reports' );
		$my_profile 		= get_page_by_title( 'My Profile' );		
		$approve_user 		= get_page_by_title( 'Approve User' );
        ...
        // WPJM
        $job_submit 	= get_page_by_title( 'Post Job');
        $job_dashboard 	= get_page_by_title( 'Job Dashboard');
        $job_page 		= get_page_by_title( 'Jobs');
        $job_alerts 	= get_page_by_title( 'Job Alerts');
		...
		$company_submit 	= get_page_by_title( 'Submit Company');
		$company_dashboard 	= get_page_by_title( 'Company Dashboard');
        $company_page 		= get_page_by_title( 'Companies');
        ...
        // WPJM RESUMES
        $resume_submit 		= get_page_by_title( 'Submit Resume');
        $resume_dashboard 	= get_page_by_title( 'Candidate Dashboard');
        $resume_page 		= get_page_by_title( 'Resumes');
        ...
        $shop_cart 		= get_page_by_title( 'Cart' );
        $shop_checkout 	= get_page_by_title( 'Checkout' );
        $shop_account 	= get_page_by_title( 'My Account' );
...
function cariera_importer_tpl() {
	$status = Cariera_Onboarding::$status;
