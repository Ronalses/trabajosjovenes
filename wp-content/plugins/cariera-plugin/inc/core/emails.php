...
class Cariera_Core_Emails {
	...
	public function listing_promoted( $post_id ) {
		...
		if ( !in_array( $post->post_type, ['job_listing', 'resume', 'company'] ) || !get_option('cariera_listing_promoted_email') ) {
			return;
		}
	...
	public function listing_promotion_expired( $post_id ) {
		...
		// Return if post is not a listing or "listing notification" has been disabled
		if ( !in_array( $post->post_type, ['job_listing', 'resume', 'company'] ) || !get_option('cariera_promotion_expired_email') ) {
			return;
		}
    ...
    */
    public function resume_expired_send_email( $new_status, $old_status, $post ) {
        ...
        $message = sprintf( esc_html__( 'Hello %s,', 'cariera' ), $author->display_name ) . "\n" . "\n";
        $message .= sprintf( esc_html__( 'Your resume *%s* has now expired: %s', 'cariera' ), $post->post_title, get_permalink( $post->ID ) );
